...
conn.sendButt = async (jid, text, buttons, quoted, options = {}) => {
    try {
      ...
      const hasImage = options.image && options.image.url;
      ...
      if (hasImage) {
        ...
        if (opts.reply_to_message_id) {
          photoOptions.reply_to_message_id = opts.reply_to_message_id;
        }
        
        result = await conn.telegram.sendPhoto(jid, options.image.url, photoOptions);
      } else {
